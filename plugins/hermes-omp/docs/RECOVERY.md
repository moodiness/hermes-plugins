...
3. Run `hermes omp doctor --json`, write a bounded offline report with `hermes omp diagnose NAME --output diagnosis.json --json`, and inspect live changes with `hermes omp watch NAME --json`. Treat every report as sensitive despite redaction.
...
Export archives are portable session records, not complete confidential backups. They omit live PID fields and locks and apply heuristic redaction, yet can retain sensitive configuration, paths, prompts, executable locations, and unrecognized secrets. HMAC-SHA256 authentication can detect modification when its key is supplied by file or environment reference; it provides no encryption or confidentiality.
