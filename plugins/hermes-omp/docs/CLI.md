...
- `doctor [--fix] [--dry-run] [--json]`: inspect executables, profile state, service backend selection, stale owner locks, legacy state-lock directories, and oversized logs. With every session stopped, `--fix` atomically replaces an abandoned legacy lock directory with the persistent kernel-lock guard; live or unverifiable owners are never removed. It also safely rotates oversized inactive logs.
