...
- `doctor [--fix] [--dry-run] [--json]`: inspect executables, profile state, service backend selection, and repair only missing/private directories or stale owner locks. This is distinct from `hermes plugins doctor PATH --ci`.
...
- `stop NAME [--json]`; `restart NAME [--json]`; `remove NAME [--no-service] [--json]`.
